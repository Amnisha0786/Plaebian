...
const EditVideo = () => {
  ...
  useEffect(() => {
    if (data?.data?.video) {
      ...
      setEmpowerCoin(video?.power - video?.powerTransferred);
  ...
  const onSubmitForm = async (values) => {
    ...
    const response = await putRequest({
      API: `${API_URL}video/${values?.videoId}`,
      DATA: formData,
    });
    dispatch(stopLoading());
    if (empowerFromApi < empowerCoin) {
      addEmpowerCoin(
        response?.data?.data?.video?.id,
        empowerCoin - empowerFromApi
      );
    } else if (empowerFromApi > empowerCoin) {
      subtractEmpowerCoin(response?.video?.id, empowerFromApi - empowerCoin);
    } else {
  ...
  const addEmpowerCoin = async (videoId, power) => {
    ...
    const response = await postRequest({
      API: `${API_URL}video/addPowerToVideo`,
      DATA: body,
    });
    if (response?.error) {
  ...
  const subtractEmpowerCoin = async (videoId, power) => {
    ...
    const response = await postRequest({
      API: `${API_URL}video/removePower/${videoId}`,
      DATA: body,
    });
    if (response?.error) {
      toast(response?.error || "Something went wrong", {
